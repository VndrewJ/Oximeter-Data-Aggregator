from bleak import BleakScanner, BleakClient
import asyncio
import csv
import time
import os
import socket
import pickle

# Server stuff
HEADERSIZE = 10

sock_to_send = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
sock_to_send.bind((socket.gethostname(), 1243))
sock_to_send.listen(5)

# Bluetooth Stuff
DEVICE_ADDRESS = ""
CHAR_UUID = ""
Write_Interval = 1  # seconds
Write_Duration = 60  # How long the program runs for before haulting

# Store the latest data
latest_data = {"hr": None, "spo2": None, "timestamp": None}

# Buffer for current interval
data_buffer = []

async def find_device():
    global DEVICE_ADDRESS, CHAR_UUID

    devices = await BleakScanner.discover()

    for d in devices:

        if d.name == "BLT_M70C":  # Replace with your device name
            print(f"Found device: {d.name} - {d.address}")
            DEVICE_ADDRESS = d.address
            break

    if not DEVICE_ADDRESS:
        print("Device not found, will try again")
        return -1

    async with BleakClient(DEVICE_ADDRESS) as client:
        services = client.services
        for service in services:
            for char in service.characteristics:
                if "notify" in char.properties:
                    return char.uuid
    
    print("Device not found, will try again")

    return -1


def notification_handler(sender, data: bytearray):
    """Parse HR and SpO₂ from raw data frame."""
    global latest_data, data_buffer
    raw = list(data)

    # print(f"Raw data: {raw}")

    # Need at least 19 bytes if we are going to look at index 18
    if len(raw) < 19:
        return

    try:
        if raw[18] == 255 and not (raw[15] == 255 and raw[16] == 127 and raw[17] == 255):
            spo2 = raw[16]
            hr = raw[17]
            ts = int(time.time())
            data_buffer = [ts, hr, spo2]
            print(f"Decoded -> HR={hr}, SpO₂={spo2}")
    except Exception as e:
        print("Parse error:", e)

# async def send_data(client_socket):
#     global HEADERSIZE, data_buffer

#     while True:
#         print("Attempting to send Data")

#         msg = pickle.dumps(data_buffer)
#         header = f"{len(msg):<{HEADERSIZE}}".encode("utf-8")  # Use latin1 to preserve byte values

#         client_socket.send(header + msg)

#         print("Data sent successfully.")

#         time.sleep(Write_Interval)



async def handle_client(reader, writer):
    global HEADERSIZE, data_buffer

    print("attempting to send data")

    msg = pickle.dumps(data_buffer)
    header = f"{len(msg):<{HEADERSIZE}}".encode("utf-8")  # Use latin1 to preserve byte values
    writer.write(header + msg)
    await writer.drain()

    print(f"Data sent: {data_buffer}")
    writer.close()
    await writer.wait_closed()


    



# async def main():
#     global DEVICE_ADDRESS, CHAR_UUID, Write_Duration, sock_to_send

#     print("Searching for devices...")
#     while True:
#         CHAR_UUID = await find_device()

#         if CHAR_UUID != -1:
#             break
    
    
#     # Server code:
#     client_socket, address = sock_to_send.accept()
#     print(f'Connection from {address} has been established!')

#     async with BleakClient(DEVICE_ADDRESS) as client:
#         print("Connected. Subscribing to notifications...")
#         await client.start_notify(CHAR_UUID, notification_handler)

#         # Run data sender in parallel
#         writer_task = asyncio.create_task(send_data(client_socket))

#         # Keep connection alive (adjust duration as needed)
#         await asyncio.sleep(Write_Duration)

#         await client.stop_notify(CHAR_UUID)
#         writer_task.cancel()
#         client_socket.close()
#         print("Stopped.")

## TEST CASE ##
async def main():
    global DEVICE_ADDRESS, CHAR_UUID, Write_Duration

    print("Searching for devices...")
    while True:
        CHAR_UUID = await find_device()
        if CHAR_UUID != -1:
            break

    server = await asyncio.start_server(handle_client, "0.0.0.0", 1243)

    async with BleakClient(DEVICE_ADDRESS) as client:
        print("Connected. Subscribing to notifications...")
        await client.start_notify(CHAR_UUID, notification_handler)

        async with server:
            await asyncio.sleep(Write_Duration)
            await client.stop_notify(CHAR_UUID)
            await server.serve_forever()

asyncio.run(main())
